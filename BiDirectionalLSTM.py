...
class BiLSTM(nn.Module):
    def __init__(self, batch_size,input_size=1, hidden_size=64, output_size=2):
        ...
        # functionals
        self.bi_lstm = nn.LSTM(input_size=input_size, hidden_size=self.hidden_layer_size, num_layers=1,
                               batch_first=True, dropout=0.5,bidirectional=True)
        self.linear1 = nn.Linear(in_features=2 * hidden_size, out_features=self.hidden_layer_size, bias=True)
        self.uni_lstm = nn.LSTM(input_size=hidden_size, hidden_size=self.hidden_layer_size, num_layers=1, dropout=0.5,
                                batch_first=True)
