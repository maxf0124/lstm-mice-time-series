...
class MiceLoader:
    ...
    def get_cc_data_in_dict(self, CCLines, time_interval, embedding):
        """
        ...
        line_idx = self.ccd.find_attr_by_value('line', self.CCLines)
        # generate dictionary
        self.data_dict = {name: self.get_mice_data(idx)
                          for name, idx in zip(self.ccd.get_attr_values('mouse_id', idx=line_idx), line_idx)}
        return self.data_dict

    def get_cc_data_in_dict_full(self, CCLines):
        """
        This function takes in CClines and return dictionary contains the full time series data
        :param CCLines: A list of CC lines
        :return: dict_ts: A dictionary: key values are mouse names, each item is a 1-d array(full time series
                 corresponding to that mouse)
        """
        self.CCLines = CCLines
        line_idx = self.ccd.find_attr_by_value('line', self.CCLines)
        self.data_dict_ts = {name: self.get_full_time_series(idx) for name, idx
                             in zip(self.ccd.get_attr_values('mouse_id', idx=line_idx), line_idx)}
    ...
    def get_mice_data(self, idx):
        ...
        This function takes in an index in self.ccd and output list: [Data,Label,Interval]
                 ...
                 Interval: (#samples, time_interval) 2-d list: [start:end] start to end of the time series relative to
                            inoculation time
        :param idx: index of data points in ccd data set
    ...
    def time_delayed_embedding(self, x, raw_idx):
        ...
        :param raw_idx: time index for original data, (n,) vector
